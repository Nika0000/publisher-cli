...
create index if not exists idx_versions_channel_published
  on publisher.versions(release_channel, is_published, created_at desc);

create index if not exists idx_versions_min_supported
  on publisher.versions(min_supported_version)
  where min_supported_version is not null;

comment on column publisher.versions.release_channel is 'Release lane: stable, beta, alpha';
comment on column publisher.versions.storage_key_prefix is 'Storage namespace prefix (e.g. releases/stable/1.2.0)';
