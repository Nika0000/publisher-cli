...
async function buildVersionManifest(version: string, channel: string) {
  const { data: versionData, error: versionError } = await appDb()
    .from('versions')
    .select('*')
    .eq('version_name', version)
...
export async function generateLatestManifest(channel: string) {
  try {
    ...
    const selectedBuildBySource = new Map<string, any>();
...
function rankDistribution(distribution: string): number {
  return distribution === 'store' ? 0 : 1;
}

function mapBuildRowToPlatformPayload(build: any) {
  return {
    os: build.os,
    arch: build.arch,
    type: build.type,
